...
test.describe("Booking page", () => {
  ...
  test("empty submit shows validation", async ({ page }) => {
    ...
    await expect(page.getByText(/Please fill name, email, and service details/i)).toBeVisible();
  });
  ...
  test("happy path makes one call and shows success", async ({ page }) => {
    ...
    const [req] = await Promise.all([
      ...
      page.getByRole("button", { name: /submit booking request/i }).click(),
    ]);
    await expect(page.getByText(/Thanks! Your booking request was sent/i)).toBeVisible();
